...
export class MapService {
    ...
    private geocodeLocation(location: string): Observable<any> {   //func to get lat & lng from map-API

        if(!this.geoCoder) {
            this.geoCoder = new (<any>window).google.maps.Geocoder();   //Geocode() is a class for map-API   
        }

        return new Observable((observer)=> {

        this.geoCoder.geocode({address: location}, (result, status) => {      //geocode() is a method
    ...
    // main function calling from map.Component to get coordinates from Google API 
